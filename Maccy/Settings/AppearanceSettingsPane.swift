...
struct AppearanceSettingsPane: View {
  ...
  var body: some View {
    Settings.Container(contentWidth: 650) {
      ...
      Settings.Section(label: { Text("Preview") }) {
        Defaults.Toggle(key: .openPreviewAutomatically) {
          Text("OpenPreviewAutomatically", tableName: "AppearanceSettings")
        }
        Text("Show a larger preview of a clip when you hover over it in the list.")
          .controlSize(.small)
      ...
      Settings.Section(label: { Text("Show in Menu Bar & Window") }) {
        ...
        Text("Show the text of your most recently copied item next to the menu-bar icon.")
          .controlSize(.small)
          .foregroundStyle(.gray)
          .fixedSize(horizontal: false, vertical: true)
        HStack {
